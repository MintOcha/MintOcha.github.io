...
this.$stat=$(eHTML(["<div role=\"complementary\" aria-label=\"","\"></div>"],TL(["Active Pok\xE9mon"])));
this.$fx=$('<div></div>');
this.$leftbar=$(eHTML(["<div class=\"leftbar\" role=\"complementary\" aria-label=\"","\"></div>"],TL(["Your team"])));
this.$rightbar=$(eHTML(["<div class=\"rightbar\" role=\"complementary\" aria-label=\"","\"></div>"],TL(["Opponent's team"])));
this.$turn=$('<div></div>');
...
var unrevealedIllusion=BattleLog.escapeHTML(TL(["Unrevealed Illusion user"]));
...
var notRevealed=BattleLog.escapeHTML(TL(["Not revealed"]));
...
turnsLeft=function turnsLeft(min){var max=arguments.length>1&&arguments[1]!==undefined?arguments[1]:0;
if(max){
var range=TL.orList([""+min,""+max]);
return TL(["("," turns)"],range);
}
return min===1?TL(["("," turn)"],min):TL(["("," turns)"],min);
};_proto.
pseudoWeatherLeft=function pseudoWeatherLeft(pWeather){
var buf="<br />"+TL(Dex.moves.get(pWeather[0]));
...
var condition=TL(Dex.moves.get(cond[0]));
var foeCondition=TL(["Foe's ",""],condition);
var buf="<br />"+(isFoe&&!all?foeCondition:condition);
if(this.battle.gen<7&&this.battle.hardcoreMode)return buf;
...
var turnText=BattleLog.escapeHTML(this.log.turnText(""+this.battle.turn));
...
var turnText=BattleLog.escapeHTML(this.log.turnText(""+turn));
var $newTurn=$("<div class=\"turn has-tooltip\" data-tooltip=\"field\" data-ownheight=\"1\">"+turnText+"</div>");
$newTurn.css({
opacity:0,
left:160
});
